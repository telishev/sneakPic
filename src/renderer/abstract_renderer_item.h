...
protected:
  ...
  bool m_bbox_valid;
public:
  abstract_renderer_item (const string &name);
  ...
  void push_back_child (const string &child);

  void set_unique_id (int id) { m_unique_id = id; }
  int unique_id () const { return m_unique_id; }

  void erase_child (const string &child);

  void set_parent (const string &parent) { m_parent = parent; }
