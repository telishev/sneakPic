...
public:
  base_anchor_handle (svg_path_iterator path_it, svg_item_path *item);
  ~base_anchor_handle ();

  void set_path_iterator (svg_path_iterator path_it);
  virtual QPointF get_handle_center () const;
  void set_node_type (node_type_t node_type) { m_node_type = node_type; }

  svg_path_iterator path_iterator () const { return m_path_it; }
