 ...
 *
 */
static QPointF Bezier(QPointF *V, int degree, double t, QPointF *Left, QPointF *Right)
{
 ...
 */
static double ComputeXIntercept (QPointF *V, int degree)
{
 ...
 *
 */
static int ControlPolygonFlatEnough(QPointF *V, int degree)
{
 ...
 *
 */
static int CrossingCount (QPointF *V, int degree)
{
    int     i;
 ...
 */
static QPointF *ConvertToBezierForm (QPointF P, QPointF *V)
{
    int     i, j, k, m, n, ub, lb;
    ...
    for (row = 0; row <= DEGREE - 1; row++) {
        for (column = 0; column <= DEGREE; column++) {
            cdTable[row][column] = geom::dot (d[row], c[column]);
        }
    }
 ...
 *    of roots found.
 */
static int FindRoots (QPointF *w, int degree, double *t, int depth)
{
    int     i;
 ...
 *
 */
QPointF path_nearest_point::get_nearest_point (QPointF P, QPointF V[4], double *t_p /*= nullptr*/)
{
    ...
    /* Finally, look at distance to end point, where t = 1.0 */
        new_dist = geom::distance2 (P, V[DEGREE]);
            if (new_dist < dist) {
                dist = new_dist;
            t = 1.0;
        }
    ...
    if (t_p)
      *t_p = t;
    return (Bezier(V, DEGREE, t, (QPointF *)NULL, (QPointF *)NULL));
}

static inline QRectF get_bbox (QPointF *points, size_t size)
{
...
double path_nearest_point::get_nearest_point (QPointF screen_pos, QTransform transform, const svg_path_geom *path,
                                               double *t_p, svg_path_geom_iterator *res_it_p)
{
  const double max_dist = 20;
  double cur_dist = max_dist;
  double t = -1.0;
  svg_path_geom_iterator cur_it = path->end ();
  ...
  for (svg_path_geom_iterator it = path->begin (); it != path->end (); ++it)
    {
      ...
      bezier[1] = segment.c1;
      bezier[2] = segment.c2;
      bezier[3] = segment.end;
      for (auto &&point : bezier)
        point = transform.map (point);

      QRectF bbox = get_bbox (bezier, 4).adjusted (-cur_dist, -cur_dist, cur_dist, cur_dist);
      if (!bbox.contains (screen_pos))
        continue;

      double cur_t = -1.0;
      QPointF cur_near_point = get_nearest_point (screen_pos, bezier, &cur_t);
      double dist = geom::distance (cur_near_point, screen_pos);
      if (dist < cur_dist)
        {
          cur_dist = dist;
          t = cur_t;
